...
@WebServlet(name = "AdminModifyTypeServlet", urlPatterns = "/admin/AdminModifyTypeServlet")
public class AdminModifyTypeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            form.setT_id(fileItemList.get(1).getString("UTF-8")); //将一级分类id存入Type
            form.setT_name(fileItemList.get(2).getString("UTF-8")); //将一级分类名称存入Type

            if (form.getT_name().trim().equals("")) {
                throw new TypeException("分类名称不能为空!");
            }
            /** 校验该一级分类是否已存在 */
            Type type = typeService.findTypeByName(form.getT_name());
            if (type != null && !type.getT_id().equals(form.getT_id())) {
                throw new TypeException("该一级分类已存在!");
            }
            type = typeService.findTypeById(form.getT_id());
            ...
            form.setT_image(type.getT_image());
            if (!fileItemList.get(3).getName().equals("")) {
                int begin = fileItemList.get(3).getName().indexOf(".");
                String suffix = fileItemList.get(3).getName().substring(begin); //截取图片后缀
                ...
                if (!filename.toLowerCase().endsWith("svg")) {
                    ...
                    request.getRequestDispatcher("/adminjsps/admin/type/modifyType.jsp").forward(request, response);
                    return;
                }
                ...
                /** 删除原有图片 */
                String[] strs = type.getT_image().split("/");
                String path = "C:\\Users\\炸弹人\\IdeaProjects\\lexian-webservice\\type_img\\" + strs[strs.length-1];
